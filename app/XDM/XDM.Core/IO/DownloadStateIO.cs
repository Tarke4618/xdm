...
namespace XDM.Core.IO
{
    ...
    public static class DownloadStateIO
    {
        ...
        private static SingleSourceHTTPDownloaderState SingleSourceHTTPDownloaderStateFromBytes(BinaryReader r)
        {
            ...
            {
                Id = r.ReadString(),
                TempDir = StreamHelper.ReadString(r),
            ...
            if (r.ReadBoolean())
            {
                ...
                {
                    Host = StreamHelper.ReadString(r),
                    Port = r.ReadInt32(),
                    ProxyType = (ProxyType)r.ReadInt32(),
                    UserName = StreamHelper.ReadString(r),
                    Password = StreamHelper.ReadString(r),
                };
            }
        ...
        private static void StateToBytes(SingleSourceHTTPDownloaderState state, BinaryWriter w)
        {
            ...
            if (hasHeaders)
            {
                StreamHelper.WriteStateHeaders(state.Headers, w);
            }
            var hasCookies = state.Cookies != null;
            w.Write(hasCookies);
            if (hasCookies)
            {
                StreamHelper.WriteStateCookies(state.Cookies, w);
            }
        ...
        private static void StateToBytes(DualSourceHTTPDownloaderState state, BinaryWriter w)
        {
            ...
            if (hasHeaders1)
            {
                StreamHelper.WriteStateHeaders(state.Headers1, w);
            }
            var hasHeaders2 = state.Headers2 != null;
            w.Write(hasHeaders2);
            if (hasHeaders2)
            {
                StreamHelper.WriteStateHeaders(state.Headers2, w);
            }
            var hasCookies1 = state.Cookies1 != null;
            w.Write(hasCookies1);
            if (hasCookies1)
            {
                StreamHelper.WriteStateCookies(state.Cookies1, w);
            }
            var hasCookies2 = state.Cookies2 != null;
            w.Write(hasCookies2);
            if (hasCookies2)
            {
                StreamHelper.WriteStateCookies(state.Cookies2, w);
            }
            var hasProxy = state.Proxy.HasValue;
            w.Write(hasProxy);
            if (hasProxy)
        ...
        private static MultiSourceDASHDownloadState MultiSourceDASHDownloadStateFromBytes(BinaryReader r)
        {
            ...
            {
                Id = r.ReadString(),
                TempDirectory = StreamHelper.ReadString(r),
                FileSize = r.ReadInt64(),
                Demuxed = r.ReadBoolean(),
                SpeedLimit = r.ReadInt32(),
                AudioChunkCount = r.ReadInt32(),
                AudioContainerFormat = StreamHelper.ReadString(r),
                VideoChunkCount = r.ReadInt32(),
                VideoContainerFormat = StreamHelper.ReadString(r),
        ...
        private static MultiSourceHLSDownloadState MultiSourceHLSDownloadStateFromBytes(BinaryReader r)
        {
            ...
            var muxedPlaylistUrl = StreamHelper.ReadString(r);
            var nonMuxedAudioPlaylistUrl = StreamHelper.ReadString(r);
            var nonMuxedVideoPlaylistUrl = StreamHelper.ReadString(r);
